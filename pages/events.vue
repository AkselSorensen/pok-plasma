    ...
    <div class="grid grid-cols-2 gap-4">
      <div v-for="e in events" :key="e.id"
        ...
        <div class="text-[10px] font-bold uppercase tracking-widest mb-2"
          ...
          {{ e.type === 'AIRDROP' ? 'Airdrop' : e.type === 'RECOLTE' ? 'Recolte' : e.type === 'FISHING' ? 'Fishing' : e.type === 'TEAMHUNT' ? 'TeamHunt' : 'MegaRaid' }}
        </div>
        ...
        <div class="flex items-center gap-4 text-[11px] text-[#5a5e66]">
          <span v-if="e.location" class="flex items-center gap-1"><Icon name="map-pin" :size="12" /> {{ e.location }}</span>
          <span class="flex items-center gap-1"><Icon name="clock" :size="12" /> {{ e.status === 'ACTIVE' ? 'En cours' : e.status === 'UPCOMING' ? 'A venir' : 'Termine' }}</span>
          <span class="ml-auto text-[10px] font-semibold px-2 py-0.5 rounded-full"
            ...
            {{ e.status === 'ACTIVE' ? 'En cours' : e.status === 'UPCOMING' ? 'A venir' : 'Termine' }}
          </span>
        </div>
      </div>
    </div>
